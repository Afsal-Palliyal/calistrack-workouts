...
export function Dashboard() {
   ...
   useEffect(() => {
      // Gate 1: Wait for auth to resolve
      if (isAuthLoading) {
         return;
      }

      // Gate 2: If no user, stop loading so it doesn't spin forever
      ...
      const unsubscribe = onSnapshot(
         ...
         (snapshot) => {
            ...
            snapshot.docs.forEach((doc) => {
               ...
               if (data.exercises && Array.isArray(data.exercises)) {
                  data.exercises.forEach((ex) => {
                     const vol =
                        (parseInt(ex.reps) || 0) * (parseInt(ex.sets) || 1);
                     dayTotal += vol;

                     // Track core movements for volume trend
                     if (ex.name === "Pushups")
                        dailyTotals[formattedDate].Pushups += vol;
                     if (ex.name === "Pullups")
                        dailyTotals[formattedDate].Pullups += vol;
                     if (ex.name === "Dips")
                        dailyTotals[formattedDate].Dips += vol;
                  });
               }
               ...
               if (
                  isAfter(new Date(docDate), new Date(startOfCurrentWeek)) ||
                  new Date(docDate).toISOString() === startOfCurrentWeek
               ) {
            ...
            // Only remove the skeleton loader if this data came from the live database,
            // ignoring the initial empty local cache flash.
            if (!snapshot.metadata.fromCache || snapshot.docs.length > 0) {
               setLoading(false);
            } else {
               // Failsafe for brand new users with 0 workouts who navigate back and forth
   ...
   // Global Auth Loading Guard
   if (isAuthLoading) {
      return (
         <div className="space-y-6">
            <header className="flex items-center justify-between mt-2">
               <div className="animate-pulse h-10 w-48 bg-white/10 rounded-xl"></div>
               <div className="animate-pulse bg-[#1A1A1A] p-3 rounded-[20px] border border-white/10 w-14 h-14"></div>
            </header>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 lg:gap-6">
                  <Card className="flex flex-col items-center justify-center p-6 lg:p-8 bg-[#1A1A1A] border border-white/10 h-full animate-pulse min-h-[160px]"></Card>
                  <Card className="flex flex-col items-center justify-center p-6 lg:p-8 bg-[#1A1A1A] border border-white/10 h-full animate-pulse min-h-[160px]"></Card>
               </div>
               <div className="lg:col-span-2">
                  <Card className="h-full flex flex-col min-h-[350px] bg-[#1A1A1A] animate-pulse"></Card>
               </div>
            </div>
         </div>
      );
      ...
      <div className="space-y-6">
         <header className="flex items-center justify-between mt-2">
            <div>
               ...
               <p className="text-textMuted font-medium tracking-wide mt-1">
                  Welcome back
               </p>
            </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-4 lg:gap-6">
               <Card className="flex flex-col items-center justify-center p-6 lg:p-8 bg-[#1A1A1A] border border-white/10 transition-all group cursor-default h-full">
                  ...
                  />
                  {loading ? (
                     <div className="w-16 h-12 bg-white/10 animate-pulse rounded-xl mb-1" />
                  ) : (
                     <span className="text-5xl font-bold text-white">
                        {weeklyReps}
                     </span>
                  )}
                  <span className="text-[12px] font-bold text-primary uppercase tracking-widest mt-3 text-center opacity-80">
                     Weekly Reps
                  </span>
               </Card>
               <Card className="flex flex-col items-center justify-center p-6 lg:p-8 bg-[#1A1A1A] border border-white/10 transition-all hover:bg-white/5 group cursor-pointer h-full">
                  ...
                  <span className="text-[12px] font-bold text-accent uppercase tracking-widest mt-3 text-center opacity-80">
                     Workouts (30D)
                  </span>
               </Card>
            </div>
                           ...
                           <ResponsiveContainer width="100%" height="100%">
                              <LineChart data={chartData}>
                                 ...
                                 <Line
                                    ...
                                    style={{
                                       filter:
                                          "drop-shadow(0px 4px 8px rgba(14, 165, 233, 0.4))",
                                    }}
                                 />
                                 <Line
                                    name="Pullups"
                                 ...
                                 <Line
                                    ...
                                    style={{
                                       filter:
                                          "drop-shadow(0px 4px 8px rgba(192, 38, 211, 0.4))",
                                    }}
                                 />
                              </LineChart>
                           </ResponsiveContainer>
